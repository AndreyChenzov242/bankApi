    ...
    var _BuildDataModel = function ( obj, numberOfFields, inputName ) {
      var pinly_input;
       ...
       for ( i = 0; i < numberOfFields; i++ ) {
         pinly_input = $( '<input type="number" class="' + inputName + '" pattern="[0-9]*" value="0" min="0" max="9" alt="Your browser does not support javascript" data="' + i + '">' );
	 ...
	 var _ValidateInput = function ( obj, max, e ) {   
	    ...
	    if( isNaN( String.fromCharCode( e.charCode ) ) || // Invalid if not a numeric character
			 ...
			 // Prevent default actions and bubbling on keydown
                 ...
                 keyup: function( e ) {
                  ...
                  // tab (no shift) OR right arrow moves right to left  
                    ...
                    e.preventDefault(); 
                      ...
                      if ( $( '.' + plugin.options.highlight ).length === $( '.' + plugin.options.input_name ).length ) {
                        $( '.' + plugin.options.input_name ).removeClass( plugin.options.highlight ).val( 0 );
                      ...
                      if ( obj.next().hasClass( plugin.options.input_name ) && obj.next().length !== 0 ) 
			            _DetermineMovement( obj, plugin.options.input_name, true );
			      ...
			      focus: function() {
                    save = $( this ).val();
                  	...
                  	$( this ).addClass( plugin.options.highlight )
                    		 .val( '' );
			      },
		 	  ...
		 	  plugin.element.on({
		 	  	pinlyFail : function ( e ) {
			 	  	$( '.' + plugin.options.input_name, plugin.element ).addClass( plugin.options.highlight );
		 	  	},
		 	  	...
		 	  	pinlyPass : function ( e ) {
			 		...
			 		$( '.' + plugin.options.input_name ).addClass( plugin.options.on_pass )
